        ...
        while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
            ...
            <div class="post">
                ...
                if ($_SESSION['ruolo'] == 'docente' and $_SESSION['email'] == $line['email']) {
                    ?>
                    <div class="del"><a
                                href="cancellaPost.php?post=<?php echo $line['idpost'] ?>&annod_id=<?php echo $corso ?>"><i
                                    class="fas fa-trash-alt"
                                    style="margin-right:10%; text-decoration:none; color:black;"></i></a></div>
                <?php } ?>
                ...
                <div class="testoPost"><?php echo $line['testo']; ?> <br><br>
                    <?php $i = $line['idpost'];
                    if ($line['codice'] != null and $line['linguaggio'] != null) {
                        ?>

                        <div id="iconaCode<?php echo $i ?>" style="font-size:20px; width:fit-content;cursor:pointer"><b>Codice
                                Allegato</b> <i class="fa fa-code" aria-hidden="true"></i></div>
                        <div id="textCode<?php echo $i ?>" class="animate slideIn">
                            <div style="background-color:#282a36; color:#fff; width:100px; text-align:center"
                                 id="ling<?php echo $i ?>"><?php echo $line['linguaggio']; ?></div>
                            <textarea name="editor" id="editor<?php echo $i ?>" style="resize: none"
                                      readonly> <?php echo $line['codice'] ?></textarea></div>
                ...
                <div class="data">
                    ...
                    while ($linefile = pg_fetch_array($result8, null, PGSQL_ASSOC)) { ?>
                        <a href="<?php echo $linefile['url']; ?>" download="<?php echo $linefile['name']; ?>"
                           style="text-decoration:none">
                            <div class="attache"><?php echo $linefile['name']; ?> <i class="fas fa-paperclip"></i></div>
                        </a>
                    <?php }
                    ?>
                </div>
...
<script>
    ...
    $('document').ready(function () {
        ...
        window.addEventListener('click', function (e) {
            ...
            if (document.getElementById('tondoChat').contains(e.target)) {
                $('#chat').show();
                var objDiv = document.getElementById("containerMex");
    ...
    function stampa(el) {
        ...
        if (el.mittente == mio) {
            var mex = "<div class='messaggio'><div class='immagineMia'><img src='img_docente/" + el.mittente + ".png' style='width: 100%;margin-bottom:40px'></div><div class='textmsgMio'>" + el.testo + "</div><div class='nomeMio'>" + el.nome + "<br><i class='fas fa-clock'></i>" + ora + "&nbsp;&nbsp;&nbsp;<i class='fas fa-calendar-alt'></i>" + data + "</div></div>";
        } else {
            var mex = "<div class='messaggio'><div class='immagineTua'><img src='img_docente/" + el.mittente + ".png' style='width: 100%;margin-bottom:40px'></div><div class='textmsgTuo'>" + el.testo + "</div><div class='nomeTuo'>" + el.nome + "<br><i class='fas fa-clock'></i>" + ora + "&nbsp;&nbsp;&nbsp;<i class='fas fa-calendar-alt'></i>" + data + "</div></div>";
        }
        return mex;
